...
Original file is located at
    https://colab.research.google.com/drive/1H1eZvkI6Ip7oRTHKb5Jln9N26uigwMi_
"""

# -*- coding: utf-8 -*-
"""
DRIVING METRICS PIPELINE — FULL FINAL CODE
Google Colab-ready:  upload a single **Final_Merged_Trajectory*.csv** file
Produces:
  • Bar plots per metric            → /BarPlots
  • Scatter plots of averages       → /ScatterPlots_Averages
  • Box-plots by vehicle type       → /VehicleType_Distribution_Plots
  • Table_2_CorrelationMatrix.csv
  • Table_4_GeneralizedVIF.csv
  • Driving_Metrics_AllOutputs.zip  (everything zipped + downloaded)
"""
...
plt.rcParams["figure.dpi"] = 200   # sharper plots

# ───────────────────────── 3. UPLOAD CSV & NORMALISE HEADERS ────────────────
uploaded   = files.upload()                       # choose the CSV
file_name  = list(uploaded.keys())[0]
df         = pd.read_csv(io.BytesIO(uploaded[file_name]))

# >>> ONE-LINER FIX: make spaces → underscores, strip blanks
df.columns = df.columns.str.strip().str.replace(' ', '_', regex=False)

# ───────────────────────── 4. VEHICLE TYPE TAG ──────────────────────────────
df["VehicleType"] = df["VehicleID"].astype(str).str.extract(r"([A-Za-z]+)")

# ───────────────────────── 5. PER-VEHICLE METRICS ───────────────────────────
results = []
...
# ───────────────────────── 12. BOX-PLOTS BY VEHICLE TYPE ────────────────────
veh_types = df["VehicleType"].unique()

# 12a. Deviation from centreline
