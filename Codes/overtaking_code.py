# -*- coding: utf-8 -*-
"""Overtaking_Code.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1KWQE1owfEhOGCn8Luq0Tfo_Rz2vySssy
"""

# ===============================================================
#  TWO-LANE UNDIVIDED · SIGNED-DEVIATION OVERTAKE ANALYZER v6-fix
#  – data-driven thresholds & broadcast-safe gap computation –
# ===============================================================
...
for fast_id in veh_ids:
    ...
    for slow_id in veh_ids:
        ...
        if not (gaps[0] - gaps.min() > lane_thr_m and gaps.min() < proximity_m):
            continue   # no decisive close-in

        # ── lane crossing with noise suppression ─────────────────────
        dev  = fast_win[signed_col].values
        sign = np.sign(np.clip(dev, -lane_thr_m, lane_thr_m))
        cross_pts = np.where(np.diff(sign) != 0)[0]
        if cross_pts.size == 0: continue     # never crossed centre-line

        ci = cross_pts[0]                    # first cross
