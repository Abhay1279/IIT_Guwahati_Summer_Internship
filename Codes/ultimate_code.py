...
Original file is located at
    https://colab.research.google.com/drive/1ovwchI-kjzFs2a9I94lyN41IHaoSQatL
"""

# -*- coding: utf-8 -*-
"""FINAL_MERGED_TRAJECTORY_AND_CURVE_WIDENING_WITH_FILTERS.ipynb
-----------------------------------------------------------------
Auto-downloads:
...
# 1 ▌ INSTALL & IMPORT
# ────────────────────────────────────────────────────────────────
# !pip install -q tqdm        # ← uncomment first time in Colab
...
# 2 ▌ UPLOAD CSV FILES
# ────────────────────────────────────────────────────────────────
uploaded = files.upload()           # drag-and-drop the 3 CSVs
...
# Coerce numeric road coordinates
...
def acute_angle(deg: float) -> float:
    ...
    if deg > 180: deg = 360 - deg   # 0–180 (mirror)
    if deg > 90:  deg = 180 - deg   # 0–90 (acute)
    return deg
...
# ────────────────────────────────────────────────────────────────
# 4 ▌ BUILD ROAD ARRAYS & DENSIFY (0.5 m)
# ────────────────────────────────────────────────────────────────
...
for vid in tqdm(vehicle_df["VehicleID"].unique(), desc="Processing vehicles"):
    ...
    # 6-A. centre-line deviation ±3.5 m
    ...
    # 6-D. heading & yaw  (ACUTE 0-90 °)
    vx, vy       = data["X_Smt"].diff().fillna(0), data["Y_Smt"].diff().fillna(0)
    heading_raw  = np.degrees(np.arctan2(vx, vy))           # signed –180…180
    theta_abs    = np.vectorize(acute_angle)(heading_raw)   # 0-90
    ...
    # 6-E. nearest tangent & acute curve-angle (0-90 °)
    ...
    # --- relative / final angle (also 0-90 because both inputs are 0-90) ---
    ...
    # 6-H. assemble per-row dataframe
    df_out = pd.DataFrame({
...
# ────────────────────────────────────────────────────────────────
# 7 ▌ SAVE MERGED CSVs & EVASIVE SUMMARY
# ────────────────────────────────────────────────────────────────
final_df = pd.concat(rows_out, ignore_index=True)

main_cols = [  # (unchanged list of columns) ...
...
# ────────────────────────────────────────────────────────────────
# 10 ▌ OVERVIEW OF ALL PROCESSED VEHICLES
# ────────────────────────────────────────────────────────────────
plt.figure(figsize=(7,6))
plt.plot(road_df["Left_E"], road_df["Left_N"],"g--", label="Left edge")
plt.plot(road_df["Right_E"],road_df["Right_N"],"r--", label="Right edge")
