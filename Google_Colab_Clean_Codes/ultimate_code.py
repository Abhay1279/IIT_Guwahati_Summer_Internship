...
import io, os, shutil, zipfile, warnings
...
vehicle_file    = next(f for f in uploaded if "smoothen"   in f.lower())
road_file       = next(f for f in uploaded if "homography" in f.lower())
centreline_file = next(f for f in uploaded if "centreline" in f.lower())

vehicle_df = pd.read_csv(io.BytesIO(uploaded[vehicle_file]))
road_df    = pd.read_csv(io.BytesIO(uploaded[road_file]))
centre_df  = pd.read_csv(io.BytesIO(uploaded[centreline_file]), header=None)

num_cols = ["Left_N","Left_E","Right_N","Right_E"]
road_df[num_cols] = road_df[num_cols].apply(pd.to_numeric, errors="coerce")

def acute_angle(deg: float) -> float:
    """Return an angle in the closed interval 0–90 °."""
    deg = (deg + 360) % 360         
...
def densify_polyline(xy, step=0.5):
    """Return a densified copy of polyline xy at <step> m spacing."""
...
os.makedirs("/content/Vehicle_Trajectories",    exist_ok=True)
os.makedirs("/content/YawRate_Plots",           exist_ok=True)
os.makedirs("/content/VehicleType_Projections", exist_ok=True)
...
for vid in tqdm(vehicle_df["VehicleID"].unique(), desc="Processing vehicles"):
    ...
    for p in pts:
        ...
        bearing_raw = math.degrees(math.atan2(ab[1], ab[0]))  # signed
        acute       = acute_angle(bearing_raw)                # 0-90
    ...
    plt.tight_layout(); plt.savefig(f"/content/Vehicle_Trajectories/{vid}_traj.png"); plt.close()
    ...
    plt.tight_layout(); plt.savefig(f"/content/YawRate_Plots/{vid}_yaw_deg.png"); plt.close()
...
final_df.to_csv("/content/Final_Merged_Trajectory.csv", index=False)
pd.DataFrame(evasive_rows).to_csv("/content/Evasive_Time_Summary.csv", index=False)
...
pd.DataFrame(entropy_rows).to_csv("/content/Steering_Metrics_Final_Cleaned.csv", index=False)

shutil.make_archive("/content/Vehicle_Trajectories","zip","/content/Vehicle_Trajectories")
shutil.make_archive("/content/YawRate_Plots","zip","/content/YawRate_Plots")
...
for vt in ["Car","Bike","LCV","Truck"]:
    ...
    plt.tight_layout()
    plt.savefig(f"/content/VehicleType_Projections/{vt}_projections.png"); plt.close()
shutil.make_archive("/content/VehicleType_Plots","zip","/content/VehicleType_Projections")
...
for v in processed_ids:
    ...
    sub = vehicle_df[vehicle_df["VehicleID"]==v]
    plt.plot(sub["X_Smt"], sub["Y_Smt"], lw=.7, alpha=.5, color=c,
             label=vtype(v))
...
plt.tight_layout(); plt.savefig("/content/All_Vehicle_Trajectories.png"); plt.close()

for fn in [
    "Final_Merged_Trajectory.csv",
    "Evasive_Time_Summary.csv",
    "Steering_Metrics_Final_Cleaned.csv",
    "Vehicle_Trajectories.zip",
    "YawRate_Plots.zip",
    "VehicleType_Plots.zip",
    "All_Vehicle_Trajectories.png"
]:
    files.download(fn)
