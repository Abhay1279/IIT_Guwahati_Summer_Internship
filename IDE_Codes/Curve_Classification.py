...
centreline_path = input("Enter path to centreline CSV: ").strip()
homography_path = input("Enter path to homography CSV: ").strip()
trajectory_path = input("Enter path to merged/smoothed trajectory CSV: ").strip()

centreline_df = pd.read_csv(centreline_path, header=None)
...
homography_df = pd.read_csv(homography_path)
veh_df = pd.read_csv(trajectory_path)
...
if idx_trans1_start > 0:
    ...
    plt.plot(lx, ly, lw=2, color="royalblue")
    plt.plot(rx, ry, lw=2, color="royalblue")
...
plt.plot(lx, ly, lw=2, color="orange")
plt.plot(rx, ry, lw=2, color="orange")
...
plt.plot(lx, ly, lw=2, color="red")
plt.plot(rx, ry, lw=2, color="red")
...
plt.plot(lx, ly, lw=2, color="green")
plt.plot(rx, ry, lw=2, color="green")
...
print("✓ Done! Output files saved as:", output_csv, "and", fig_name)
