...
veh_file = input("Enter path to Final_Merged_Trajectory.csv: ").strip()
df = pd.read_csv(veh_file)
...
lane_thr_m = (np.percentile(noise_band, 90) if len(noise_band) else 0.05)
proximity_m = 1.1 * (2 * half_lane)

print(f'λ half-lane≈{half_lane:.2f} m | noise±{lane_thr_m:.2f} m | proximity={proximity_m:.2f} m')
...
for fast_id in veh_ids:
    ...
    for slow_id in veh_ids:
        ...
        if slow['Direction'].iat[0] != fast_dir: continue
        t0, t1 = max(fast['Time'].iat[0], slow['Time'].iat[0]), min(fast['Time'].iat[-1], slow['Time'].iat[-1])
        if t1 <= t0: continue
...
out = os.path.join(downloads, "Final_Overtaking_Summary_With_Overtaken_ID_and_Coords.csv")
summary.to_csv(out, index=False)
print("✓ Completed –", len(summary), "overtakes detected")
print("Output file saved to:", out)
